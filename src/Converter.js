...
function Converter() {
  ...
  const deafultSecondSelectValue = "Ether";

  const [CryptoList, setCryptoList] = useState([]);
  const [inputValue, setInputValue] = useState("0");
  const [firstSelect, setFirstSelect] = useState(defaultFirstSelectValue);
  const [secondSelect, setSecondSelect] = useState(deafultSecondSelectValue);
  ...
  async function fetchData() {
    ...
    // Using For Each Loop to Convert Object into Arraylist
    // const tempArray = [];
  ...
  return (
    <div className="container">
      ...
      <Card className="crypto-card" title ={<h1> <BsCoin/> Crypto Converter</h1>}>
        <Form size="large">
          <Form.Item>
        ...
        <div className="select-box">
          ...
          <Select 
            style={{width: "200px"}} 
            defaultValue={deafultSecondSelectValue} 
            options={CryptoList}
